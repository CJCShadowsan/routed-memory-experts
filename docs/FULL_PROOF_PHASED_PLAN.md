...
**Current selected benchmark:** GSM8K (`openai/gsm8k`, MIT license, Hugging Face provenance) is the first reviewed public benchmark. The committed sample is `workloads/gsm8k_public_sample.jsonl`, generated by `scripts/build-gsm8k-public-workload.py` from the test split.
...
**Stop condition:** If no live server is available locally, provide `scripts/run-openai-public-benchmark.sh` and instructions instead of fabricating results. GSM8K is now available locally as the reviewed workload sample; a live server and quality-matched adapter are still external requirements for adapter-quality claims.
